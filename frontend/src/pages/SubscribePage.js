  ...
  return (
    <div className="min-h-screen bg-[#FDFBF7]">
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        {isSubscribed ? (
          <div className="bg-white border border-[#E4DFD5] rounded-xl p-8 text-center">
            ...
            <p className="text-[#7A8A76] mb-6">
              You have an active subscription. Enjoy unlimited lesson plans!
            </p>
            <button
          ...
          <>
            ...
            <div className="grid sm:grid-cols-2 gap-6 mb-8">
              {plans.map((plan) => (
                ...
                >
                  ...
                  <div className="mb-6">
                    <span className="text-4xl font-bold text-[#1A2E16]">{plan.currency} {plan.price}</span>
                    <span className="text-[#7A8A76]">/{plan.period}</span>
                  </div>

                  <ul className="space-y-3">
                    {plan.features.map((feature, i) => (
                      <li key={i} className="flex items-center gap-3 text-[#4A5B46]">
            ...
            <div className="text-center">
              <button
                ...
                disabled={loading}
                className="bg-[#D95D39] text-white px-12 py-4 rounded-xl font-semibold text-lg hover:bg-[#BD4D2D] transition-colors shadow-lg disabled:opacity-50"
                data-testid="subscribe-now-btn"
              >
                {loading ? 'Processing...' : 'Subscribe Now'}
              </button>
              <p className="text-[#7A8A76] text-sm mt-4">
                Demo Mode • Real payment will be processed via PesaPal
              </p>
            </div>
          </>
        )}
