...
const fetchAndDownload = async (url, filename, fileId, setDownloadingFiles) => {
  try {
    // Set loading state
    if (setDownloadingFiles) {
...
const MyFiles = () => {
  ...
  const [downloadingFiles, setDownloadingFiles] = useState({}); // {fileId: true}
  ...
  const generateTextContent = (lesson) => {
    ...
    text += `\nGenerated by mi-lessonplan.site\n`;
    return text;
  };
             ...
             <div className="flex items-center justify-between">
               <span className="text-xs text-[#7A8A76] flex-shrink-0">{new Date(file.created_at).toLocaleDateString()}</span>
                <div className="flex items-center gap-2 flex-shrink-0">
                  ...
                  <button onClick={() => fetchAndDownload(`${API_URL}/api/lessons/${file.lesson_id}/export`, `${file.subject}_${file.topic}_lesson.pdf`, `lesson-${file.lesson_id}`, setDownloadingFiles)} className="flex items-center gap-1 text-xs text-[#8E44AD] font-medium hover:text-[#6C3483] disabled:opacity-50" data-testid={`download-lesson-${file.lesson_id}`} disabled={downloadingFiles[`lesson-${file.lesson_id}`]}>
                    {downloadingFiles[`lesson-${file.lesson_id}`] ? (
                      <><div className="w-3.5 h-3.5 border-2 border-[#8E44AD] border-t-transparent rounded-full animate-spin" />Loading</>
                    ) : (
                      <><Download className="w-3.5 h-3.5" />PDF</>
                    )}
                  </button>
                  {/* Image download option for Arabic lessons */}
                  {(file.subject && (file.subject.toLowerCase().includes('arabic') || file.subject.includes('اللغة العربية') || file.subject.includes('عربي'))) && (
                    <button onClick={() => fetchAndDownload(`${API_URL}/api/lessons/${file.lesson_id}/export/image`, `${file.subject}_${file.topic}_lesson.png`, `lesson-image-${file.lesson_id}`, setDownloadingFiles)} className="flex items-center gap-1 text-xs text-[#D95D39] font-medium hover:text-[#B8431F] disabled:opacity-50" data-testid={`download-lesson-image-${file.lesson_id}`} disabled={downloadingFiles[`lesson-image-${file.lesson_id}`]}>
                      {downloadingFiles[`lesson-image-${file.lesson_id}`] ? (
                        <><div className="w-3.5 h-3.5 border-2 border-[#D95D39] border-t-transparent rounded-full animate-spin" />Loading</>
                      ) : (
          ...
          <div className="pt-3 border-t border-[#E4DFD5]">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2 flex-wrap justify-end">
                ...
                <button
                  onClick={() => fetchAndDownload(`${API_URL}/api/dictations/${file.dictation_id}/download`, `${file.title || 'dictation'}.mp3`, `dictation-${file.dictation_id}`, setDownloadingFiles)}
                  className="flex items-center gap-1 text-xs text-[#8E44AD] font-medium hover:text-[#6C3483] disabled:opacity-50"
                  data-testid={`download-dictation-${file.dictation_id}`}
                  disabled={downloadingFiles[`dictation-${file.dictation_id}`]}
                >
                  {downloadingFiles[`dictation-${file.dictation_id}`] ? (
                    <><div className="w-3.5 h-3.5 border-2 border-[#8E44AD] border-t-transparent rounded-full animate-spin" />Loading</>
                  ) : (
          ...
          <div className="flex items-center justify-between pt-3 border-t border-[#E4DFD5]">
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
              {file.content_type?.startsWith('image/') || file.type?.includes('image') ? (
                ...
                <button onClick={() => fetchAndDownload(`${API_URL}/api/uploads/${file.upload_id}/download`, file.name, `upload-${file.upload_id}`, setDownloadingFiles)}
                  className="flex items-center gap-1 text-xs text-[#8E44AD] font-medium hover:text-[#6C3483] disabled:opacity-50" data-testid={`download-upload-${file.upload_id}`} disabled={downloadingFiles[`upload-${file.upload_id}`]}>
                  {downloadingFiles[`upload-${file.upload_id}`] ? (
                    <><div className="w-3.5 h-3.5 border-2 border-[#8E44AD] border-t-transparent rounded-full animate-spin" />Loading</>
                  ) : (
                    <><Download className="w-3.5 h-3.5" />Download</>
                  )}
          ...
          <div className="pt-3 border-t border-[#E4DFD5]">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                ...
                <button onClick={() => fetchAndDownload(`${API_URL}/api/templates/${file.template_id}/export`, `${file.name || 'template'}.pdf`, `template-${file.template_id}`, setDownloadingFiles)} className="flex items-center gap-1 text-xs text-[#8E44AD] font-medium hover:text-[#6C3483] disabled:opacity-50" data-testid={`download-template-${file.template_id}`} disabled={downloadingFiles[`template-${file.template_id}`]}>
                  {downloadingFiles[`template-${file.template_id}`] ? (
                    <><div className="w-3.5 h-3.5 border-2 border-[#8E44AD] border-t-transparent rounded-full animate-spin" />Loading</>
                  ) : (
                    <><Download className="w-3.5 h-3.5" />Download</>
                  )}
        ...
        <div className="pt-3 border-t border-[#E4DFD5]">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
              ...
              <button onClick={() => fetchAndDownload(`${API_URL}/api/schemes/${file.scheme_id}/export`, `Scheme_${file.subject || 'untitled'}_${file.syllabus}.pdf`, `scheme-${file.scheme_id}`, setDownloadingFiles)}
                className="flex items-center gap-1 text-xs text-[#8E44AD] font-medium hover:text-[#6C3483] disabled:opacity-50" data-testid={`download-scheme-${file.scheme_id}`} disabled={downloadingFiles[`scheme-${file.scheme_id}`]}>
                {downloadingFiles[`scheme-${file.scheme_id}`] ? (
                  <><div className="w-3.5 h-3.5 border-2 border-[#8E44AD] border-t-transparent rounded-full animate-spin" />Loading</>
                ) : (
                  <><Download className="w-3.5 h-3.5" />PDF</>
                )}
