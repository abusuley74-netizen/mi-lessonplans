...
class Admin(BaseModel):
    ...
    is_active: bool = True
    tasks: List[str] = []  # Assigned dashboard sections/tasks
    created_at: datetime = Field(default_factory=lambda: datetime.now(timezone.utc))
    updated_at: datetime = Field(default_factory=lambda: datetime.now(timezone.utc))
...
class AdminTask(BaseModel):
    task_id: str
    admin_id: str
    task_name: str  # dashboard section name
...
class PesaPalTransaction(BaseModel):
    ...
    ipn_traces: List[Dict[str, Any]] = []  # Track all IPN calls
    created_at: datetime = Field(default_factory=lambda: datetime.now(timezone.utc))
    updated_at: datetime = Field(default_factory=lambda: datetime.now(timezone.utc))

class UserManagement(BaseModel):
    user_id: str
    action: str  # block, unblock, suspend, activate, delete
...
async def admin_login(request: Request, response: Response):
    ...
    expires_at = datetime.now(timezone.utc) + timedelta(hours=8)  # Shorter admin sessions
    ...
    return {"admin": admin.model_dump(), "message": "Admin login successful"}
...
async def get_dashboard_data(current_admin: Admin = Depends(get_current_admin)):
    ...
    return {
        ...
        "recent_activity": {
            ...
            "lessons_created_7d": recent_lessons
        },
        "referrals": {
...
async def get_navigation(current_admin: Admin = Depends(get_current_admin)):
    """Get navigation menu filtered by admin permissions"""
    all_sections = [
        {"id": "dashboard", "name": "Dashboard", "icon": "📊", "path": "/admin/dashboard"},
        {"id": "referral_registry", "name": "Referral Registry", "icon": "🤝", "path": "/admin/referral-registry"},
        {"id": "refer_and_earn", "name": "Refer and Earn", "icon": "💰", "path": "/admin/refer-and-earn"},
        {"id": "user_management", "name": "User Management", "icon": "👥", "path": "/admin/users"},
        {"id": "content_management", "name": "Content Management", "icon": "📚", "path": "/admin/content"},
        {"id": "analytics", "name": "Analytics & Reports", "icon": "📈", "path": "/admin/analytics"},
        {"id": "advanced_reports", "name": "Advanced Reports", "icon": "📊", "path": "/admin/reports"},
        {"id": "subscription_management", "name": "Subscription Management", "icon": "💳", "path": "/admin/subscriptions"},
        {"id": "template_management", "name": "Template Management", "icon": "📝", "path": "/admin/templates"},
        {"id": "communication", "name": "Communication", "icon": "💬", "path": "/admin/communication"},
        {"id": "promo_banner", "name": "Promo Banner", "icon": "🎯", "path": "/admin/promo"},
        {"id": "admin_profiles", "name": "Admin Profiles", "icon": "👥", "path": "/admin/profiles"}
    ]
    ...
    # For regular admins, filter by assigned tasks
    allowed_sections = []
    for section in all_sections:
        if check_admin_permission(current_admin, section["id"]):
...
async def manage_user(
    ...
    """Manage user account (block, unblock, suspend, activate, delete)"""
    if not check_admin_permission(current_admin, "user_management"):
        raise HTTPException(status_code=403, detail="No permission for user management")
    ...
    if action_data.action == "block":
        ...
        update_data["blocked_at"] = datetime.now(timezone.utc)
        ...
        update_data["suspended_reason"] = action_data.reason
        ...
        update_data["deleted_reason"] = action_data.reason
    else:
...
async def get_revenue_analytics(current_admin: Admin = Depends(get_current_admin)):
    ...
    if not check_admin_permission(current_admin, "advanced_reports"):
        raise HTTPException(status_code=403, detail="No permission for advanced reports")
...
async def get_all_referrals(
    ...
    """Get all referrals across all admins"""
    if not check_admin_permission(current_admin, "referral_registry"):
        raise HTTPException(status_code=403, detail="No permission for referral registry")
...
async def get_referral_stats(current_admin: Admin = Depends(get_current_admin)):
    """Get referral statistics across all admins"""
    if not check_admin_permission(current_admin, "referral_registry"):
        raise HTTPException(status_code=403, detail="No permission for referral registry")
    ...
    admin_stats = await db.referrals.aggregate([
        {"$group": {
            ...
            "active_referrals": {"$sum": {"$cond": [{"$eq": ["$status", "active"]}, 1, 0]}}
        }},
...
async def get_subscription_data(current_admin: Admin = Depends(get_current_admin)):
    ...
    if not check_admin_permission(current_admin, "subscription_management"):
        raise HTTPException(status_code=403, detail="No permission for subscription management")
    ...
    subscription_stats = await db.users.aggregate([
        {"$group": {
            "_id": "$subscription_plan",
            "count": {"$sum": 1},
            "active": {"$sum": {"$cond": [{"$eq": ["$subscription_status", "active"]}, 1, 0]}},
            "expired": {"$sum": {"$cond": [{"$eq": ["$subscription_status", "expired"]}, 1, 0]}}
        }},
        {"$sort": {"count": -1}}
    ]).to_list(10)
...
async def update_subscription_plans(
    ...
    data = await request.json()
    # In production, this would update a plans configuration
...
async def get_content_lessons(
    current_admin: Admin = Depends(get_current_admin),
    skip: int = 0,
    limit: int = 50
):
    """Get lessons for content moderation"""
    if not check_admin_permission(current_admin, "content_management"):
        raise HTTPException(status_code=403, detail="No permission for content management")
    ...
    return {"lessons": lessons, "total": total, "skip": skip, "limit": limit}
...
async def delete_content_lesson(
    ...
    """Delete inappropriate lesson content"""
    if not check_admin_permission(current_admin, "content_management"):
        raise HTTPException(status_code=403, detail="No permission for content management")
...
async def send_user_message(
    ...
    if not check_admin_permission(current_admin, "communication"):
        raise HTTPException(status_code=403, detail="No permission for communication")
...
async def get_promo_banners(current_admin: Admin = Depends(get_current_admin)):
    ...
    if not check_admin_permission(current_admin, "promo_banner"):
        raise HTTPException(status_code=403, detail="No permission for promo banner management")
...
async def get_pesapal_transactions(
    request: Request,
    current_admin: Admin = Depends(get_current_admin)
):
    """Get all PesaPal transactions for admin review"""
    if not check_admin_permission(current_admin, "subscription_management"):
        raise HTTPException(status_code=403, detail="No permission for payment management")
    ...
    transactions = await db.pesapal_transactions.find(
        query,
        {"_id": 0}
...
async def get_pesapal_transaction_details(
    merchant_reference: str,
    current_admin: Admin = Depends(get_current_admin)
):
    """Get detailed PesaPal transaction information"""
    if not check_admin_permission(current_admin, "subscription_management"):
        raise HTTPException(status_code=403, detail="No permission for payment management")

    transaction = await db.pesapal_transactions.find_one(
...
async def get_pesapal_analytics(
    current_admin: Admin = Depends(get_current_admin)
):
    """Get PesaPal payment analytics"""
    if not check_admin_permission(current_admin, "analytics"):
        raise HTTPException(status_code=403, detail="No permission for analytics")
    ...
    thirty_days_ago = datetime.now(timezone.utc) - timedelta(days=30)
    revenue_trend = await db.pesapal_transactions.aggregate([
        {"$match": {
            "status": "COMPLETED",
...
async def calculate_commission(plan: str, months: int) -> float:
    """Calculate commission based on plan and active months"""
    plan_prices = {
        "free": 0,
        "basic": 9999,
        "premium": 19999,
    ...
    return price * months * commission_rate
...
async def get_referral_metrics(admin_id: str) -> ReferralMetrics:
    """Get referral metrics for an admin"""
    referrals = await db.referrals.find(
        {"admin_id": admin_id},
        {"_id": 0}
    ).to_list(1000)
    
    total_teachers = len(referrals)
    total_commission = sum(r.get("total_commission", 0) for r in referrals)
    active_teachers = len([r for r in referrals if r.get("status") == "active"])
...
async def sync_admin_referrals(admin_id: str):
    """Sync admin referrals data"""
    # This function would handle syncing with any external systems
    ...
    # Update any stale commission calculations
    for referral in referrals:
        current_plan = referral.get("subscription_plan", "free")
        active_months = referral.get("active_months", 0)
        expected_commission = await calculate_commission(current_plan, active_months)
        
        if abs(referral.get("total_commission", 0) - expected_commission) > 0.01:
            await update_referral_commission(
...
def _build_pesapal_request_data(reference: str, amount: int, description: str, user: User):
    ...
    return f"""
<PesapalDirectOrderInfo xmlns=\"http://www.pesapal.com\">
...
async def _create_pesapal_checkout_url(reference: str, amount: int, description: str, user: User) -> str:
    ...
    # Response is expected to be a URL (iframe src). Example: https://www.pesapal.com/API/PostPesapalDirectOrderV4?...
...
async def generate_lesson(
    ...
    if user.subscription_status == "free":
        plan = "free"
    elif user.subscription_status == "active" and plan not in ["basic", "premium", "enterprise"]:
        plan = "basic"  # fallback for legacy active users
    ...
    current_limit = limits.get(plan, 20)

    if current_limit is not None:
        lesson_count = await db.lesson_plans.count_documents({"user_id": user.user_id})
        if lesson_count >= current_limit:
            raise HTTPException(
                status_code=403,
                detail=f"{plan.capitalize()} plan limit reached ({current_limit} lessons). Please upgrade to continue generating lessons."
            )
        ...
        "plans": [
            {
                ...
                "features": [
                    ...
                    "Print lesson plans",
                    "Core lesson planning functionality"
                ],
                "limitations": [
                    "No MyHub access",
                    "No file uploads",
                    "No resource sharing",
                    "No scheme of work creation",
                    "No notes creation",
                    "No templates access",
            ...
            },
            {
                ...
                "limitations": [
                    "Limited resource sharing (vs Premium)"
                ]
            },
            {
                "id": "premium",
            ...
            {
                "id": "enterprise",
                "name": "Enterprise Plan (Referral only)",
                "price": 29999,
                "currency": "TZS",
                "period": "month",
                "lesson_limit": None,
                "features": [
                    "Enterprise-grade team support",
                    "Custom SLA",
                    "Dedicated onboarding",
                    "Custom integrations"
                ],
                "limitations": [
                    "Not available for direct user subscription",
                    "Referral-only entry"
                ],
                "referral_only": True
            }
...
async def subscription_checkout(request: Request, user: User = Depends(get_current_user)):
    """Initiate a PesaPal checkout for a plan."""
    data = await request.json()
...
@app.post("/listentowebsitepaymentsipn.php")
async def pesapal_ipn(request: Request):
    """Handle PesaPal instant payment notifications (IPN) with enhanced security."""
    ...
    # Log IPN call for debugging
    logger.info(f"PesaPal IPN received: tracking_id={transaction_tracking_id}, merchant_ref={merchant_reference}, status={transaction_status}")

    if not merchant_reference:
        logger.warning("PesaPal IPN: Missing merchant reference")
        raise HTTPException(status_code=400, detail="Missing merchant reference")

    # Validate transaction status
    valid_statuses = ["PENDING", "COMPLETED", "FAILED", "CANCELLED", "VALID", "INVALID"]
    if transaction_status and transaction_status.upper() not in valid_statuses:
        logger.warning(f"PesaPal IPN: Invalid status '{transaction_status}'")
    ...
    if not tx:
        logger.warning(f"PesaPal IPN: Transaction not found for merchant_ref={merchant_reference}")
        raise HTTPException(status_code=404, detail="Transaction not found")

    # Create IPN trace entry
    ipn_trace = {
        "timestamp": datetime.now(timezone.utc).isoformat(),
        "tracking_id": transaction_tracking_id,
        "status": transaction_status,
        "ip_address": request.client.host if request.client else "unknown",
        "user_agent": request.headers.get("user-agent", "unknown")
    }

    # Update transaction with IPN trace and status
    update_data = {
        "updated_at": datetime.now(timezone.utc).isoformat()
    }
    ...
    # Add IPN trace to the array
    await db.pesapal_transactions.update_one(
        {"merchant_reference": merchant_reference},
        {
            "$set": update_data,
            "$push": {"ipn_traces": ipn_trace}
        }
    )
    ...
    if transaction_status and transaction_status.upper() in ["COMPLETED", "VALID"]:
        ...
        referral = await db.referrals.find_one({"teacher_id": user_id}, {"_id": 0})
        if referral:
            await update_referral_commission(referral["referral_id"], plan_id, 1)

        logger.info(f"PesaPal IPN: Successfully processed payment for user {user_id}, plan {plan_id}")

    elif transaction_status and transaction_status.upper() in ["FAILED", "CANCELLED", "INVALID"]:
        logger.info(f"PesaPal IPN: Payment {transaction_status} for merchant_ref={merchant_reference}")
...
async def subscribe(request: Request, user: User = Depends(get_current_user)):
    """Fallback local plan activation (not used when PesaPal is enabled)."""
    data = await request.json()
    plan_id = data.get("plan_id")
    
    if plan_id not in ["basic", "premium"]:
        raise HTTPException(status_code=400, detail="Invalid plan or not available for direct subscription")
    
    expires = datetime.now(timezone.utc) + timedelta(days=30)
    plan_name = "Basic" if plan_id == "basic" else "Premium"
    ...
    if referral:
        active_months = 1
        await update_referral_commission(referral["referral_id"], plan_id, active_months)

    return {
        "message": f"{plan_name} subscription activated (FALLBACK)",
...
async def create_referral(request: Request, user: User = Depends(get_current_user)):
    ...
    required_fields = ["teacher_id", "teacher_name", "teacher_email", "admin_id", "admin_name"]
...
async def get_admin_referrals(admin_id: str, user: User = Depends(get_current_user)):
    """Get all referrals for an admin"""
    # Basic permission check - in production, add proper admin role checking
    if user.user_id != admin_id and not user.email.endswith("@admin.com"):  # Simple admin check
...
async def update_referral(referral_id: str, request: Request, user: User = Depends(get_current_user)):
    ...
    # Check permissions
    if user.user_id != existing["admin_id"] and not user.email.endswith("@admin.com"):
    ...
    allowed_fields = ["subscription_plan", "monthly_price", "active_months", "inactive_months", "status"]
    for field in allowed_fields:
        if field in data:
            update_data[field] = data[field]
    
    # Recalculate commission if plan or months changed
...
async def get_referral_metrics_endpoint(admin_id: str, user: User = Depends(get_current_user)):
    ...
    if user.user_id != admin_id and not user.email.endswith("@admin.com"):
...
async def sync_admin_referrals_endpoint(admin_id: str, user: User = Depends(get_current_user)):
    ...
    # Basic permission check
    if user.user_id != admin_id and not user.email.endswith("@admin.com"):
        raise HTTPException(status_code=403, detail="Not authorized to sync these referrals")
...
async def renew_subscriptions_cron(current_admin: Admin = Depends(get_current_admin)):
    """Admin endpoint to manually trigger subscription renewal (normally called by cron job)"""
    if current_admin.role != "super_admin":
